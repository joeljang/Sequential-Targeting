...
class Wheel(object):
    ...
    def train(self,savemodel):
        ...
        oddlayers =['conv1d.weight',
            ...
            'uni_rnn.weight_ih_l0',
            'uni_rnn.weight_hh_l0',
            'uni_rnn.bias_ih_l0',
            'uni_rnn.bias_hh_l0',
            'linear.weight',
            'linear.bias']
        for j in range(no_of_task):
            ...
            '''
            #Reseting model parameters and freezing some layers!
            with torch.no_grad():
                if(j%2==1):
                    odd=True
                else:
                ...
                if(j!=0):
                    for n,p in self.model.named_parameters():
                        if(n in oddlayers):
                            if(odd):
                                self.model.state_dict()[n].requires_grad=False
                            else:
                                self.model.state_dict()[n].copy_(zero[n])
                        else:
                            if(odd):
                                self.model.state_dict()[n].copy_(zero[n])
                            else:
            ...
            for epoch in range(max_epoch):
                ...
                for i, batch in tq_iter:
                    ...
                    pred = self.model(batch.syllable_contents, j)
                ...
                acc_lst, total, prec,recall,f1score,f1s,rocauc = self.eval(j, len(self.task[j].te_dataset),trainloss=trainloss, epoch=epoch)
                
                
                self.save_model(savemodel,self.model, j, f'e{epoch}')

            #Copying model wegiths and saving fisher matrix
            if(self.type=='imm'):
                #copy weights
                copy={}
                params = {n: p for n, p in self.model.named_parameters()}
    ...
    def evalall(self, j):
        for i in range(len(self.task)):
            ...
            self.model.eval()
            for i, batch in tq_iter:
                preds = self.model(batch.syllable_contents, j)
                label = torch.tensor(batch.label, dtype=torch.long, device=self.device)
                if(self.isbinary):
